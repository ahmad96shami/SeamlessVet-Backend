...
/// <summary>
/// Computes a doctor's batch entitlement and writes it as a <see cref="DoctorEntitlement"/> row
/// (M9 task 8; M30 — batch-only, immutable accrual audit). Idempotent: re-running for the same batch
/// refreshes the row. The settlement workflow calls this when a batch is settled; the same amount is
/// credited to the doctor's <see cref="DoctorPartner"/> ledger. Honours the toggle (SCHEMA invariant #4
/// — disabled ⇒ all profit to the clinic, computed 0).
/// </summary>
public interface IEntitlementService
{
    /// <summary>Computes (and persists, once) the responsible doctor's batch entitlement — the
    /// supervision fee under the batch's <c>entitlement_system</c> (M28). Returns the persisted row.</summary>
